...
传入的依赖必须为响应式数据  
`useEffect`回调会在页面上的元素重新绘制前执行（不是交互触发的，比如新的数据更新），但是，也会在元素重新绘制后执行（比如交互触发的更新）
`useEffect`中的回调可以返回一个函数，这个函数是`cleanup`函数，可以执行一些清除副作用影响的操作

PS. [Effect 不是必须的](https://zh-hans.react.dev/learn/you-might-not-need-an-effect)  
如何理解呢，如果使用`useEffect`的组件不需要响应式数据，就不要用这个 Hook  
还有一点，React 的函数式组件会在数据更新后过后重新执行，所以搭配这个Hook 就需要注意
...
PS. 可以把 useEffect Hook 看做如下三个函数的组合
...
[官方文档 useEffect](https://zh-hans.react.dev/reference/react/useEffect)
