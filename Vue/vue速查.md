...
#### 如何判断compute数据要更新（也就是compute内提供的回调何时执行）
- 要读取 `compute`定义的响应式数据
- 内部要标记 `compute`定义的数据为脏数据（初始化compute时为脏数据，compute内部依赖的响应式数据变化时为脏数据）

#### 两个vue版本的有何不同
优化在哪？优化在需要读取 `compute`定义的数据的地方多加了一层逻辑，先执行一次 `compute`回调，内部获取到新的值，之后判断是否是脏数据
